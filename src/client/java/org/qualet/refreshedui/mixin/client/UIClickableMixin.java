...
public abstract class UIClickableMixin
{
    ...
    private void refreshedui$pressSkin(UIClickable<?> self, UIContext context)
    {
        ...
        matrices.pushMatrix();
        matrices.translate(area.x + area.w / 2F, area.y + area.h / 2F);
        matrices.scale(scale, scale);
        matrices.translate(-(area.x + area.w / 2F), -(area.y + area.h / 2F));
        /* 1.21.11: no shader colour — the darkening rides on GuiAlpha's shade, applied as the face is recorded */
        GuiAlpha.push(1F, shade);
